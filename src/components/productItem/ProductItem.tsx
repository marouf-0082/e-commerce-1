...
function ProductItem({ name, price, image }: IProductItem) {
  return (
    <div className="shadow-md rounded-3xl hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
      ...
      <div className="p-4">
        <div>
          <h3 className="inline font-[500] hover:text-[#f69e0a] transition-colors duration-300">{name}</h3>
          <h3 className="font-semibold text-[20px]">${price.toFixed(2)}</h3>
        </div>
        <button className="bg-[#f69e0a] w-full rounded-3xl py-1.5 mt-2">
          Add to Cart
        </button>
      </div>
    </div>
